...
- ape run read_value --network ethereum:sepolia ( để deploy lên sepolia)
- ape run read_value --network ethereum:local ( để deploy lên local)
- dùng (ape console) để mở console shell của apeworx. có thể tương tác trực tiếp như python shell

Lesson 6:  Brownie Fund Me 5:06:22 - 5:19:22
- Học cách mappings để import thư viện từ trên repo github import "@chainlink/contracts/src/v0.8/shared/interfaces/AggregatorV3Interface.sol";
- remappings để chỉ ra khi dùng @chainlink thì nó sẽ tự hiểu rằng lên repo "https://github.com/smartcontractkit/chainlink" để tải và import thư viện
- Safe math library check uint256 for integer overflows nhưng mà với version > 8. thì k cần nữa Solidity tự nhận diện được 
- Cài "ape plugins install etherscan" là plugin để public code lên etherscan của APEworx
- Chưa public source lên ethereum được bởi vì ethereum k tìm được file AggregatorV3Interface.sol trong .cache của contracts. chưa biết tại sao
- Cách mock AggregatorV2V3Interface để lấy giá fake của priceFeed để chạy trên local (tạo file mock trong folder test của contracts và dùng interface mock của chainlink)
- lấy tên của network đang active: active_network = networks.active_provider.network.name
- setting bên config như thế nào cũng được, mình có thể truy vấn qua để lấy thông tin
- PriceFeed là để lấy giá thực tế của token ở thời điểm hiện tại

Lesson 6:  Brownie Fund Me 5:19:22 - 5:55:00
- Withdraw và fund tài sản vào ví bằng contract đã deploy 
- Check và viết test contract đó ai là owner mới được withdraw
- Khi send token từ contract về ví thì nó được tính là internal transaction thôi
